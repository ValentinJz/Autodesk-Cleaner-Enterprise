<#
.SYNOPSIS
    Autodesk Cleaner Enterprise v1.0 - Modded by ValentinJz - Basado en Autodesk Nuke v6.3 by jcieza pero mejorado con enfoque a empresas.
    
.DESCRIPTION
    Script de limpieza de Autodesk enfocado a entornos empresariales. Disenado para resolver
    los casos mas problematicos.
...
.NOTES
    ...
    Basado en: Autodesk-Nuke Fusion de v2.0.2, v3.0-OOP, v4.0, v5.0, v6.0, v6.2
#>

param(
    [switch]$DryRun,
    [string]$LogPath = "$env:TEMP\Autodesk-Cleaner-Enterprise-v1.0_$(Get-Date -Format 'yyyyMMdd_HHmmss').log",
...
$script:CleanupLevels = @{
    "ENTERPRISE"  = @{
        Name        = "Limpieza Empresarial (Profunda)"
        Description = @"

Encuentra y elimina cualquier rastro de: Autodesk | AutoCAD | Revit | Inventor | Fusion | Civil | Maya | 3ds
...
- DryRun, Logging completo, Verificacion post-limpieza<
              ...
              -/-/-/-/-/-/-  NO ELIMINA ARCHIVOS PERSONALES NI .DWG EN EL SISTEMA  -/-/-/-/-/-/-

"@        
Duration= "5-10 minutos"
Risk= "Alto - Los cambios NO pueden deshacerse" 
...
function Initialize-Logging {
    $header = @"
================================================================================
Autodesk Cleaner Enterprise v1.0 modded by ValentinJz
================================================================================
...
function Show-Banner {
    ...
    $leftPad  = ' ' * $padding
    $rightPad = ' ' * ([math]::Max(0, $width - 2 - $padding - $Text.Length))
...
function Show-Progress {
    ...
    Write-Progress -Activity "Autodesk-Cleaner-Enterprise v1.0 modded by ValentinJz" -Status $Activity -PercentComplete $percent -Id 0
}
...
function Select-CleanupLevel {
    Show-Banner "PERFIL DE LIMPIEZA ENTERPRISE" "Cyan"
    $i = 1
    $script:CleanupLevels.Keys | ForEach-Object {
        $info = $script:CleanupLevels[$_]
        Write-Host "  $i) $($info.Name)" -ForegroundColor Cyan
        Write-Host "     $($info.Description)" -ForegroundColor Gray
        Write-Host "     Duracion Estimada: $($info.Duration)" -ForegroundColor Green -NoNewline
        Write-Host " |" -ForegroundColor White -NoNewLine
        Write-Host " Nivel de riesgo: $($info.Risk)" -ForegroundColor Red
        Write-Host ""
        Write-Host "DISCLAIMER:" -ForegroundColor Red -NoNewline
        Write-Host " Si bien el script no elimina sus archivos ni proyectos .DWG, si elimina las carpetas de Autodesk en su totalidad.
De tener archivos personales dentro de dichas carpetas, se recomienda moverlos a ubicaciones ajenas al programa antes de continuar." -ForegroundColor White
        
    }
    $keys = @($script:CleanupLevels.Keys)
    Read-Host "Para continuar presiona ENTER"
    $script:Config.CleanupLevel = $keys[0]
...
try {
    Clear-Host
    Show-Banner "Autodesk-Cleaner-Enterprise v1.0 modded by ValentinJz" "Magenta"
    Write-Host "  Disenada para los casos mas complejos y problematicos." -ForegroundColor White
    Write-Host "  Basado en Autodesk-Nuke v6.3 pero con enfoque a empresas.`n" -ForegroundColor White
    ...
    Write-Progress -Activity "Autodesk-Cleaner-Enterprise v1.0 modded by ValentinJz" -Completed
    ...
    $rep = @"
+------------------------------------------------------------------------------+
|                 Autodesk Cleaner Enterprise v1.0 - Logs                      |
+------------------------------------------------------------------------------+
